...
import { Plus, ArrowRight, Trash2, ExternalLink, BarChart3 } from "lucide-react";
import { listWaitlists, deleteWaitlist, createWaitlist } from "@/lib/waitlist-store";
...
export const Route = createFileRoute("/dashboard")({
  head: () => ({ meta: [{ title: "Dashboard — Waitly" }, { name: "description", content: "Manage your waitlist pages." }] }),
  component: Dashboard,
});
...
function Dashboard() {
  ...
  function quickCreate() {
    const w = createWaitlist(TEMPLATES[0].id);
      ...
      <header className="border-b border-border">
        <div className="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={quickCreate} className="px-4 py-2 rounded-full bg-primary text-primary-foreground text-sm font-medium inline-flex items-center gap-1.5">
                  ...
                  <Link to="/builder/$id" params={{ id: w.id }} className="block">
                    <div className="aspect-video overflow-hidden border-b border-border bg-background relative">
                      <div className="absolute inset-0 pointer-events-none">
                        <WaitlistPreview config={w.config} scale={0.32} showBranding={false} />
                      </div>
                    </div>
                  </Link>
                  <div className="p-4">
                    <div className="flex items-center justify-between">
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground truncate">/w/{w.slug}</div>
                      </div>
                    ...
                    <div className="mt-4 flex items-center gap-1.5">
                      <Link to="/builder/$id" params={{ id: w.id }} className="flex-1 text-center text-xs font-medium py-2 rounded-lg bg-foreground/10 hover:bg-foreground/20">
                        Edit
                      </Link>
          ...
          <div className="mt-10 p-5 rounded-2xl border border-border bg-surface/50 flex items-center gap-4">
            ...
            <div className="flex-1 text-sm">
              ...
              <div className="text-muted-foreground">
                {items.reduce((a, w) => a + w.signups.length, 0)} signups across {items.length} pages
              </div>
            </div>
          </div>
        )}
